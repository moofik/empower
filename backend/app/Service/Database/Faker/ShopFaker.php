...
class ShopFaker
{
     ...
     * @throws Exception
     */
    public function createFakeShopFromUser(User $user, bool $fullOptioned = false): User
    {
        $this->createShopSettings($user);
        $this->createShopSchedule($user);
        $this->createShopWarranty($user);
        $this->attachMainServices($user, $fullOptioned);
        $this->attachAdditionalServices($user, $fullOptioned);
        $this->attachBrands($user, $fullOptioned);
     ...
     * @return ShopSetting
     */
    private function createShopSettings(User $shop): ShopSetting
    {
     ...
     * @throws Exception
     */
    private function attachBrands(User $user, bool $all = false)
    {
        if ($all === false) {
